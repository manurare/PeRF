...
class CoreRunner:
    ...
    def render_dense(self, n_poses=180, cam_type='persp'):
        ...
        for i in tqdm(range(dense_pose_sampler.n_poses)):
            ...
            write_image(pjoin(out_dir, f'{i:04}_rgb.png'), colors * 255.)
            write_image(pjoin(out_dir, f'{i:04}_depth_vis.png'), colorize_single_channel_image(1. / distances))
            np.save(pjoin(out_dir, f'{i:04}_depth.npy'), distances.cpu().numpy()*SCALE)
